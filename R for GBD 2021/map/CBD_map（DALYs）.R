...
CIR_2021 <- subset(IBD,IBD$year==2021 & 
                     ...
                     IBD$measure_name=='DALYs (Disability-Adjusted Life Years)'&
                     IBD$sex_name=='Both') 
...
total <- total %>%
  mutate(val2 = cut(val, 
                    breaks = c(0,31.44,45.81,47.51,52.62,57.46,88.44,76071), 
                    labels = c("0~31.44", "31.44~45.81", 
                               "45.81~47.51", "47.51~52.62", "52.62~57.46", 
                               "57.46~88.44","88.44+"), 
                    include.lowest = TRUE, 
                    right = TRUE))
...
p <- ggplot()
p3 <- p + geom_polygon(data=total, 
                       aes(x=long, y=lat, group = group,fill=val2),
                       colour="gray",size = .01) + 
  scale_fill_manual(values = mycolor2) +
  theme_void()+labs(x="", y="")+
  guides(fill = guide_legend(title='Age-standardized DALYs Rate\nPer 100 000 population'))+
  theme(legend.position = 'right')
p3
ggsave("p3 map（DALYs ）.pdf", p3, width = 10, height = 4, dpi = 300) 