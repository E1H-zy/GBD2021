...
total <- total %>%
  mutate(val2 = cut(val, 
                    breaks = c(0,526.9,756.1,787.9,874.1,959.0,1476.3,76071), 
                    labels = c("0~526.9", "526.9~756.1", 
                               "756.1~787.9", "787.9~874.1", "874.1~959.0", 
                               "959.0~1476.3","1476.3+"), 
                    include.lowest = TRUE, 
                    right = TRUE))
...
p2 <- p + geom_polygon(data=total, 
  ...
  theme_void()+labs(x="", y="")+
  guides(fill = guide_legend(title='Age-standardized Prevalence Rate\nPer 100 000 population'))+
  theme(legend.position = 'right')
p2
ggsave("p2 map（Prevalence）.pdf", p2, width = 10, height = 4, dpi = 300) 
