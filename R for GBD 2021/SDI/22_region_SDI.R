...
setwd("D:/课程材料/代码及实操/实操/13 SDI 22地区")
library(reshape)
library(ggplot2)
library(ggrepel)
library(readxl)
...
IBD_ASMR <- subset(IBD_22, IBD_22$age_name=='Age-standardized' & 
                   ...
                   IBD_22$measure_name=='Deaths'&
                   IBD_22$sex_name=="Both")
IBD_ASMR <- IBD_ASMR[,c(5,14,15)] ###选择location year val
...
result <- cor.test(IBD_ASMR_SDI$ASMR,IBD_ASMR_SDI$SDI , method = "spearman")
summary(result)
